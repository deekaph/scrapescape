...
1. **Send tabs to the Scrapescape queue** — the current tab, every tab to the
   ...
   - **Left-click** the toolbar icon → adds the **current tab** immediately.
   - **Right-click** the icon → a native menu with *Add tabs to the right*,
     *Add all tabs*, the *Lazy background tabs* toggle, *Open Scrapescape*, and
     *Options*.
   - Results show on the icon **badge** (e.g. a green count, amber if everything
     was already queued, red `!` if the server is unreachable); hover the icon
     for the detail.
...
| `contextMenus` | The right-click menu on the toolbar icon (bulk actions + lazy toggle). Scoped to the extension's own action icon only. |
...
extension's **Options** (right-click the icon → *Options*, or the card on `brave://extensions`):
...
Toggle it from the icon's right-click menu or Options (default **OFF** until you enable it).
